...
var tabsCtrl = function($scope, globalService, $translate, $sce) {
    ...
    $scope.defaultNodeKey = 'eth_mycrypto';
    ...
    $scope.changeNode = function(key) {
        ...
        ajaxReq.getCurrentBlock(function(data) {
            if (data.error) {
                ...
                $scope.nodeIsConnected = true;
                $scope.notifier.info( globalFuncs.successMsgs[5] + '<br /> URL: <strong>' + window.location.domain + '</strong> <br /> Network: <strong>' + $scope.nodeType + ' </strong> provided by <strong>' + $scope.nodeService + '</strong>', 10000)
            }
        });
